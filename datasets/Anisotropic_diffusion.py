...
def np_random_sin(x, amplitudes, k = 2, eps = 1.0):
    ...
    for n, (i, j) in enumerate(frequencies):
        a += amplitudes[n]*sp.sin(sp.pi*i*x[0])*sp.sin(sp.pi*j*x[1]) / (1 + (sp.pi*i)**2 + (eps*sp.pi*j)**2)**k
    return a
...
def get_sample(key, coords_train, weights_, coords_legendre, coords_eval, eps = 1.0):
    ...
    phi = sp.sin(sp.pi*x)*sp.sin(sp.pi*y)*np_random_sin([x, y], amplitudes_, k = 1, eps=eps)
    f = - (a * phi.diff("x")).diff("x") - eps**2 * (a * phi.diff("y")).diff("y")
    ...
    C_F_x = 1 / (jnp.sqrt(jnp.min(a(coords.T)))*jnp.pi*2)
    C_F_y = 1 / (jnp.sqrt(jnp.min(eps*a(coords.T)))*jnp.pi*2)
    C_F = jnp.min(jnp.array([C_F_x, C_F_y]))
...
def get_dataset(key, N_samples, eps = 1.0):
    ...
    for key in keys:
        a_train_, rhs_train_, dx_a_train_, dy_a_train_, a_eval_legendre_, dx_sol_eval_legendre_, dy_sol_eval_legendre_, rhs_legendre_, a_legendre_, sol_eval_, C_F_ = get_sample(key, coords_train, weights_, coords_legendre, coords_eval, eps = eps)
...
if __name__ == "__main__":
    ...
    N_samples = 1000
    data = get_dataset(key, N_samples, eps = eps)
    jnp.savez(f"anisotropic_diffusion_{eps}.npz", **data)