...
def get_laplacian(model, x, eps):
    return grad(lambda x: grad(model, argnums=0)(x)[0])(x)[0] + eps**2*grad(lambda x: grad(model, argnums=0)(x)[1])(x)[1]

def compute_loss(model, coordinates, a, dx_a, dy_a, rhs, eps):
    flux = vmap(get_flux, in_axes=(None, 0), out_axes=1)(model, coordinates)
    laplacian = vmap(get_laplacian, in_axes=(None, 0, None))(model, coordinates, eps)
    return jnp.linalg.norm(dx_a*flux[0] + eps**2*dy_a*flux[1] + a*laplacian + rhs)
...
def make_step_scan(carry, ind, optim):
    ...
    loss, grads = vmap(compute_loss_and_grads, in_axes=(0, None, 0, 0, 0, 0, None))(model, coordinates[ind], a[:, ind], dx_a[:, ind], dy_a[:, ind], rhs[:, ind], eps)
            ...
            for N_drop in Ns_drop:
                for N_features_ in Ns_features:
                    for N_layers in Ns_layers:
                        ...
                        carry = [model, coords_train, a_train[:NN_batch], dx_a_train[:NN_batch], dy_a_train[:NN_batch], rhs_train[:NN_batch], eps, opt_state]
                        ...
                        energy_norms = jnp.sqrt(vmap(compute_error_energy_norm, in_axes=(0, None, 0, 0, 0, None, None))(model, coords_legendre, a_eval_legendre[:NN_batch], dx_sol_eval_legendre[:NN_batch], dy_sol_eval_legendre[:NN_batch], eps, weights))
