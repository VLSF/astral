...
def compute_loss(model, coordinates, rhs, a, C_F):
...
@jit
def compute_upper_bound(model, coordinates, weights, rhs, a, C_F, N_batch=10):
    flux = []
    dx_y_0 = []
    dx_y_1 = []
    y1 = []
...
@jit
def compute_error_energy_norm(model, coordinates, a, dx_sol, dy_sol, weights, N_batch=10):
    flux_ = []
    coordinates = coordinates.reshape(N_batch, -1, 2)
...
def make_step_scan(carry, ind, optim):
    model, coordinates, rhs, a, C_F, opt_state = carry
    loss, grads = vmap(compute_loss_and_grads, in_axes=(0, None, 0, 0, 0))(model, coordinates[ind], rhs[:, ind], a[:, ind], C_F)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, coordinates, rhs, a, C_F, opt_state], loss
            ...
            for N_drop in Ns_drop:
                for N_features_ in Ns_features:
                    for N_layers in Ns_layers:
                        ...
                        carry = [model, coords_train, rhs_train[:NN_batch], a_train[:NN_batch], C_F[:NN_batch], opt_state]
                        ...
                        for i in range(n_batch):
                            predicted = vmap(lambda model, coords: vmap(jit(lambda x: model(x, 2)))(coords), in_axes=(0, None))(model, coords_eval_[i])
                            predicted_.append(predicted)
                        predicted = jnp.concatenate(predicted_, 1)
                        relative_errors = jnp.linalg.norm(predicted - sol_eval[:NN_batch], axis=1) / jnp.linalg.norm(sol_eval[:NN_batch], axis=1)
                        energy_norms = jnp.sqrt(vmap(compute_error_energy_norm, in_axes=(0, None, 0, 0, 0, None))(model, coords_legendre, a_eval_legendre[:NN_batch], dx_sol_eval_legendre[:NN_batch], dy_sol_eval_legendre[:NN_batch], weights))
                        upper_bounds = jnp.sqrt(vmap(compute_upper_bound, in_axes=(0, None, None, 0, 0, 0))(model, coords_legendre, weights, rhs_legendre[:NN_batch], a_legendre[:NN_batch], C_F[:NN_batch]))
