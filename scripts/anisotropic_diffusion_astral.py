...
def compute_loss(model, coordinates, rhs, a, C_F, eps):
    ...
    loss = (1 + model.beta[0]**2)*(C_F**2*(rhs + dx_y[0] + dx_y[1])**2 +  ((a*flux[:, 0] - y1)**2 / a + (eps**2*a*flux[:, 1] - y2)**2 / (eps**2*a)) / (model.beta[0]**2))
...
def compute_upper_bound(model, coordinates, weights, rhs, a, C_F, eps, N_batch=10):
    ...
    integrand = (1 + model.beta[0]**2)*(C_F**2*(rhs + dx_y[0] + dx_y[1])**2 +  ((a*flux[0] - y1)**2 / a + (eps**2*a*flux[1] - y2)**2 / (eps**2*a)) / (model.beta[0]**2))
...
def make_step_scan(carry, ind, optim):
    ...
    loss, grads = vmap(compute_loss_and_grads, in_axes=(0, None, 0, 0, 0, None))(model, coordinates[ind], rhs[:, ind], a[:, ind], C_F, eps)
            ...
            for N_drop in Ns_drop:
                for N_features_ in Ns_features:
                    for N_layers in Ns_layers:
                        ...
                        carry = [model, coords_train, rhs_train[:NN_batch], a_train[:NN_batch], C_F[:NN_batch], eps, opt_state]
                        ...
                        energy_norms = jnp.sqrt(vmap(compute_error_energy_norm, in_axes=(0, None, 0, 0, 0, None, None))(model, coords_legendre, a_eval_legendre[:NN_batch], dx_sol_eval_legendre[:NN_batch], dy_sol_eval_legendre[:NN_batch], weights, eps))
                        upper_bounds = jnp.sqrt(vmap(compute_upper_bound, in_axes=(0, None, None, 0, 0, 0, None))(model, coords_legendre, weights, rhs_legendre[:NN_batch], a_legendre[:NN_batch], C_F[:NN_batch], eps))
