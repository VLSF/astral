...
class PiNN(eqx.Module):
    ...
    def __call__(self, x):
        ...
        f = f.at[0].set(f[0]*jnp.prod(jnp.sin(jnp.pi*x)))
        return f
...
def Astral_loss(model, x, c1, c2):
    ...
    R = compute_f(x, c1, c2)
    m = vmap(model)(x)
    dm = vmap(jacfwd(model))(x)
    for i in range(x.shape[1]):
        R += dm[:, i+1, i]
    L = jnp.mean(jnp.sum((m[:, 1:] - dm[:, 0, :])**2, axis=1))*(1+model.beta[0])/model.beta[0]
...
def compute_relative_error(model, x, c1, c2):
    return jnp.sqrt(jnp.sum((vmap(model)(x)[:, 0] - compute_solution(x, c1, c2))**2) / jnp.sum(compute_solution(x, c1, c2)**2))
...
def train(key, d, N_features_, N_layers, N_batch, N_run, N_drop, gamma_, learning_rate, N_estimate, eps):
    ...
    model = PiNN([d, N_features_, d+1], N_layers, keys[2])
...
if __name__ == "__main__":
    ...
    if not os.path.isfile("large_d_results/astral_join.csv"):
        with open("large_d_results/astral_join.csv", "w") as f:
    ...
    for N_features_, N_layers, N_drop, learning_rate in itertools.product(N_F, N_L, N_D, L_R):
        npz = hash("".join(map(str, [d, N_features_, N_layers, N_drop, learning_rate]))+"astral_join")
        training_time = []
        loss = []
        relative_error = []
        energy_norm = []
        ...
        with open("large_d_results/astral_join.csv", "a") as f:
