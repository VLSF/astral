...
class PiNN2(eqx.Module):
    ...
    def __init__(self, N_features, N_layers, key, s0=10):
        ...
        self.models = [PiNNu(N_features, N_layers, keys[0], s0=s0),]
        self.models.append(PiNN(N_features, N_layers, keys[1], s0=s0))

    def __call__(self, x, phi_k, k, a, i):
        return self.models[i](x, phi_k, k, a)

def compute_loss(model, coordinates, phi_k, k, f, a):
    du = vmap(lambda x: grad(model, argnums=0)(x, phi_k, k, a, 1))(coordinates)
    dy_dx = vmap(lambda x: grad(model, argnums=0)(x, phi_k, k, a, 0))(coordinates)[:, 0]
    y = vmap(lambda x: model(x, phi_k, k, a, 0))(coordinates)
...
def compute_upper_bound(model, coordinates, weights, phi_k, k, f, a, T, N_batch=16):
    coordinates = coordinates.reshape(N_batch, -1, 2)
    du = []
    dy_dx = []
    y = []
    for i in range(N_batch):
        du_ = vmap(lambda x: grad(model, argnums=0)(x, phi_k, k, a, 1))(coordinates[i])
        dy_dx_ = vmap(lambda x: grad(model, argnums=0)(x, phi_k, k, a, 0))(coordinates[i])[:, 0]
        y_ = vmap(lambda x: model(x, phi_k, k, a, 0))(coordinates[i])
        du.append(du_)
        dy_dx.append(dy_dx_)
        y.append(y_)
    du = jnp.concatenate(du, 0)
    dy_dx = jnp.concatenate(dy_dx, 0)
    y = jnp.concatenate(y, 0)
    r = f - du[:, 1] - a*du[:, 0] + dy_dx
    l = y - du[:, 0]
...
def compute_error_energy_norm(model, coordinates, weights, phi_k, k, a, d_exact_sol, exact_sol, T, N_batch=16):
    ...
    u_T = vmap(lambda x: model(x, phi_k, k, a, 1))(coordinates.reshape(weights.size, weights.size, 2)[:, -1, :].reshape(-1, 2))
            ...
            for N_drop in Ns_drop:
                for N_features_ in Ns_features:
                    for N_layers in Ns_layers:
                        ...
                        model = vmap(PiNN2, in_axes=(None, None, 0, None))(N_features, N_layers, keys, 10)
                        ...
                        predictions = []
                        for i in range(N_batch):
                            predictions_ = vmap(vmap(lambda model, coords, phi, a: model(coords, phi, k, a, 1), in_axes=(0, None, 0, 0)), in_axes=(None, 0, None, None), out_axes=1)(model, coords_validation_[i], phi_k[:NN_batch], a[:NN_batch])
                        ...
                        upper_bounds = vmap(compute_upper_bound, in_axes=(0, None, None, 0, None, 0, 0, None))(model, coords_leg, weights, phi_k[:NN_batch], k, f_leg[:NN_batch], a[:NN_batch], T)
