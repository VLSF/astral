...
def make_step_scan(carry, ind, optim):
    model, coordinates, a, rhs, opt_state = carry
    loss, grads = vmap(compute_loss_and_grads, in_axes=(0, None, 0, 0))(model, coordinates[ind], a[:, ind], rhs[:, ind])
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, coordinates, a, rhs, opt_state], loss
            ...
            for N_drop in Ns_drop:
                for N_features_ in Ns_features:
                    for N_layers in Ns_layers:
                        ...
                        carry = [model, coords_train, a_train[:NN_batch], rhs_train[:NN_batch], opt_state]
